<template>
  ...
  >
    <template v-slot:top>
      <v-toolbar flat >
        <v-toolbar-title>Puntuaciones</v-toolbar-title>
        <v-divider
          class="mx-4"
          inset
          vertical
  ...
  export default {
    ...
    methods: {
      initialize () {
        ...
        axios.get('http://localhost:8081/api/leaderboard/1')
        .then((response) =>{
      ...
      deleteItem (item) {
        ...
        confirm('Estas seguro que quieres eliminarlo?')
          ...
          axios.post('http://localhost:8081/api/leaderboard/delete',   this.puntuaciones[index])
      ...
      save () {
        if (this.editedIndex > -1) {
          ...
          axios.post('http://localhost:8081/api/leaderboard/update', this.editedItem)
          .then(function (response) {
            console.log(response);
          })
          .catch(function (error) {
          ...
          axios.post('http://localhost:8081/api/leaderboard/insert', this.editedItem)
          .then(function (response) {
            console.log(response);
          })
          .catch(function (error) {
